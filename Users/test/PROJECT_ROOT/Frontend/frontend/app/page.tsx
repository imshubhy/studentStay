...
// src/app/page.tsx
"use client";
// <<<<<< PAGE.TSX IS BEING REBUILT - VERSION CHECK 12345 >>>>>>
// THIS IS THE FRONTEND-ONLY VERSION CHECK - VERSION 9

import { useState, useEffect } from 'react';
import AISearchBar from '@/components/search/AISearchBar';
import PropertyList from '@/components/properties/PropertyList';
import type { PropertyCardData } from '@/lib/types';
import { mockPropertiesWithAiFeatures, mockProperties } from '@/lib/mock-data';
// Import from frontendActions.ts
import { getAiRecommendations, performAiSearch } from '@/app/frontendActions';
import type { SemanticSearchInput } from '@/app/frontendActions'; // Import type from frontendActions
...
// This function is no longer needed as frontendActions.ts directly returns PropertyCardData[]
...
export default function HomePage() {
  console.log("<<<<<< PAGE.TSX IS BEING REBUILT - VERSION CHECK 12345 >>>>>>");
    ...
    const fetchInitialData = async () => {
      ...
      if (recError) {
        ...
        setRecommendedProperties([]);
         ...
         // Recommendations are now directly PropertyCardData[]
         const aiMappedRecommendations = recommendations.map(p => ({
          ...p,
  ...
  const handleSearch = async (searchQuery: string, campus: string, appliedFilters?: Filters) => {
    ...
    // frontendActions.ts now handles basic client-side filtering internally if needed
    // and returns PropertyCardData[]
    ...
    if (searchError) {
      ...
      setProperties([]);
       ...
       // Results are now directly PropertyCardData[]
       const aiFilteredProperties = results.map(p => ({
         ...p,
  ...
  const handleApplyFilters = (filters: Filters) => {
    let count = 0;
    // Adjust default price range for active filter count comparison
       ...
       <section className="mb-16 md:my-20">
           <div className="flex items-center gap-3 mb-2">
             ...
             <h2 className="section-header !from-accent !via-primary !to-primary">
               {isLoading ? "Searching Listings..." : error ? "Search Update" : properties.length > 0 ? "Your Search Results" : "Explore Listings"}
             </h2>
           </div>
           <p className="section-subheader mb-6">
             {isLoading
